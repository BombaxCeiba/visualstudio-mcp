...
namespace VsMcp
{
    ...
    /// 周期性调用一个「发送保活」委托。
    /// <para>
    /// 该委托由调用方提供，负责构造并发送保活帧（<c>PipeToolProgress</c>）。
    /// 这些帧经 Gateway 转发为 MCP notifications/message，让客户端重置其工具调用超时（约 60s），
    /// 避免长任务被误判为断连而触发恢复。通知只显示在客户端调试面板，
    /// 不注入模型上下文、不消耗 token。
    /// </para>
    /// <para>
    /// 保活循环与主任务并行：主任务返回后调用方 <see cref="Dispose"/> 取消循环。
    /// 把「发什么」抽成委托而非硬编码，使本类可在测试中
    /// 注入计数委托验证周期行为，无需构造真实 MCP 环境。
    /// </para>
    /// </summary>
